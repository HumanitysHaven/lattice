...
| 1.5 | Invitation onboarding | Core lifecycle done ✅: single-use, expiring, identity-free invite tokens (`invite`) with authoritative issuer-side validation, encode/decode for QR/link, and Tier-0 onboarding; no stranger discovery exists. **Text-only UI landed** ✅: `app/`'s Invite screen issues a token (hex text, share out-of-band) and completes a pending one by entering the invitee's fingerprint, driving `InviteBook::redeem` → `Grant::into_contact` → the trust graph for real. `InviteBookHandle` (`app/rust`) is the FFI layer's first genuinely stateful object — one live `InviteBook` per app session, matching that its own persistence is still explicitly deferred in `core`, unlike identity/contacts. Remaining: QR display + camera scanning instead of manual text entry, and carrying redemption over the real anonymous-queue handshake (1.3/1.4) instead of manually pasting a fingerprint (`7.2`, `S11`) |
| 1.6 | Trust engine integrated | Core ingestion done ✅: signed vouches/burns (`vouching`) are signature-verified at the boundary, then drive score/tier/burn in the engine. **In-app persistence and listing landed** ✅: `app/rust::contacts` reuses `persistence::snapshot`/`restore`/`seal_graph`/`open_graph` (same stateless bytes-in/bytes-out shape as identity) so contacts survive a relaunch, and a Contacts screen lists them with their tier. Remaining: real in-app capability gating (tier unlocking group chat/file sharing/etc. in the UI) once those features themselves exist (`7.2`) |
...
Every Phase 1 milestone now has its core logic implemented, tested, and (for 1.3) verified
against live infrastructure; what's left is either UI-dependent (1.5's QR/link flow, 1.8's
app disguise) or the two items called out below (the onion service decision, MLS-for-scale).
...
`relay-client` networked edge. Real screens now cover 1.1/1.2/1.5/1.6's core lifecycles:
create-identity, restore-identity, set-passphrase, unlock, and signed-in (identity, with
local persistence — an identity survives a relaunch via `identity::seal_identity`/
`open_identity`), plus invite and contacts (issuing/completing invites and listing the
resulting trust graph, persisted the same stateless way via `persistence::seal_graph`/
`open_graph`). `InviteBookHandle` is the one stateful FFI object so far — a session-lived
`InviteBook`, matching that its own persistence is still deferred in `core`. Verified
compiling for both web (wasm) and native targets (the web path needed two real fixes along
the way: `getrandom`'s `js` feature for wasm32, and `PlatformInt64Util.from` to construct the
`i64` params `web`'s `BigInt` representation needs), with widget integration tests covering
create → persist → relaunch → unlock → forget-device, and invite → complete → contact
appears in the persisted graph (`app/integration_test`) — though, as with the rest of this
crate, these need a real device/browser to actually run, not runnable in a headless sandbox.
Messaging, trust-tier capability gating, the duress vault, and QR/camera are still unbuilt;
this is a vertical slice through several milestones' core lifecycles, not a UI pass over the
whole app.

## Immediate next actions

1. Get `threat-model.md` and `technical-spec.md` in front of an external reviewer (0.1/0.2).
2. Decide `technical-spec.md` §13's two remaining open questions (run-our-own vs. public
   relay network; offline/store-and-forward retention window) — Flutter-vs-native and the
   SimpleX-integration questions are now settled by what's built.
3. Decide whether `lattice-relay` should itself be reachable as a Tor onion service (hides the
...
5. Extend `app/`: 1:1 messaging (1.4 — a Session per contact, sent/received over a queue) is
   the natural next slice, since identity, invites, and contacts now all work; QR/camera for
   the invite flow and vouching UI (to move a contact past Tier 0) are smaller follow-ups.
